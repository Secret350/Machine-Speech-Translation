...
def calculate_bleu():
    ...
    try:
        tokenizer_en, tokenizer_vi, vectorizer_en, vectorizer_vi, idx_to_word_en, transformer = load_resource()
    ...
    print("-" * 60)
    print(f"{'INPUT (VI)':<30} | {'PREDICTION (EN)':<30} | {'BLEU':<5}")
    print("-" * 60)

    for idx in tqdm(indices):
        raw_src = vi_sentences[idx].strip()
        raw_ref = en_sentences[idx].strip()
        ...
        pred_text = beam_search(
            ...
            idx_to_word_en, transformer, beam_width=3
        )
    ...
    print(f"2. Corpus BLEU Score:     {corp_bleu * 100:.2f}")
    print("=" * 60)
